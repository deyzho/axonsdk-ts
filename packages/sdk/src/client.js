...
export class PhonixClient {
    ...
    maxPayloadBytes;
    constructor(options = {}) {
        const providerName = options.provider ?? 'acurast';
        this.secretKey =
    ...
    static createProvider(name, wsUrl) {
        switch (name) {
            case 'acurast':
    ...
    async connect() {
        if (!this.secretKey) {
            throw new PhonixError('No secret key provided. Set PHONIX_SECRET_KEY in your .env or pass secretKey to PhonixClient.');
        }
    ...
    async send(processorId, payload) {
        ...
        if (byteLength > this.maxPayloadBytes) {
            throw new PhonixError(`Payload size ${byteLength} bytes exceeds the limit of ${this.maxPayloadBytes} bytes. ` +
    ...
    async inference(options) {
        ...
        if (deployment.processorIds.length === 0) {
            throw new PhonixError('No processors were assigned to the deployment. ' +
