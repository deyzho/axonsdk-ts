...
function createProvider(name, wsUrl) {
    switch (name) {
...
export class PhonixRouter {
    ...
    eventHandlers = [];
    constructor(config) {
        const { providers, secretKey, strategy = 'balanced', processorStrategy = 'round-robin', failureThreshold = 3, recoveryTimeoutMs = 30_000, healthWindowMs = 60_000, maxRetries = 2, retryDelayMs = 200, wsUrls, } = config;
    ...
    // ─── Lifecycle ──────────────────────────────────────────────────────────────
    async connect() {
        const results = await Promise.allSettled([...this.entries.values()].map(e => e.provider.connect(this.cfg.secretKey)));
        let anyOk = false;
        ...
        if (!anyOk)
            throw new PhonixError('All providers failed to connect');
    }
    disconnect() {
    ...
    // ─── Deploy ─────────────────────────────────────────────────────────────────
    async deploy(config) {
        const results = await Promise.allSettled([...this.entries.entries()].map(async ([name, e]) => {
        ...
        if (providers.length === 0)
            throw new PhonixError('All providers failed to deploy');
    ...
    async send(payload, options = {}) {
        ...
        if (ordered.length === 0)
            throw new PhonixError('No callable providers available');
        let lastErr;
        for (let attempt = 0; attempt <= this.cfg.maxRetries; attempt++) {
            ...
            if (!entry.circuit.isCallable) {
                if (attempt > 0)
                    this._emit({ type: 'failover', provider: name, timestamp: new Date() });
                continue;
            }
            ...
            if (attempt === 0)
                this._emit({ type: 'provider:selected', provider: name, timestamp: new Date() });
            else
                this._emit({ type: 'retry', provider: name, detail: `attempt ${attempt}`, timestamp: new Date() });
            const t0 = Date.now();
            try {
