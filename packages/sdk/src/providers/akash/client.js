...
function assertSafeAkashEndpoint(endpoint) {
    let parsed;
    try {
    ...
    if (parsed.protocol !== 'https:') {
        throw new PhonixError('akash', `Lease endpoint must use https:// (got "${parsed.protocol}"). ` +
            'Plain HTTP would transmit payloads in cleartext.');
    }
    if (PRIVATE_HOST_RE.test(parsed.hostname)) {
        throw new PhonixError('akash', `Lease endpoint hostname "${parsed.hostname}" resolves to a private/internal address. ` +
...
export class AkashMessagingClient {
    ...
    async send(leaseEndpoint, payload) {
        ...
        try {
            ...
            throw new PhonixError('akash', `Failed to reach Akash container at ${leaseEndpoint}: ${err.message}`);
        }
        if (!response.ok) {
            throw new PhonixError('akash', `Container returned ${response.status}: ${await response.text()}`);
        }
        const MAX_RESULT_BYTES = 1 * 1024 * 1024; // 1 MiB
        const resultText = await response.text();
        if (resultText.length > MAX_RESULT_BYTES) {
            throw new PhonixError('akash', `Container response exceeded ${MAX_RESULT_BYTES} bytes (got ${resultText.length} bytes).`);
        }
    ...
    if (parsed !== null && typeof parsed === 'object') {
        for (const key of Object.keys(parsed)) {
            if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
                throw new PhonixError('akash', `Rejected remote payload: contains prototype-polluting key "${key}".`);
            }
        }
    }
    return parsed;
