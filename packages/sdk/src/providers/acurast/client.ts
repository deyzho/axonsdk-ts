...
function assertSecureWsUrl(url: string): void {
  if (!url.startsWith('wss://')) {
    throw new Error(
      `WebSocket URL must use the wss:// scheme to protect key material in transit. ` +
        `Received: "${url}"`
    );
  }
}
...
/** Minimal type shim for the AcurastClient from @acurast/dapp */
interface AcurastKeyPair {
  secretKey: Uint8Array;
  publicKey: Uint8Array;
}

interface AcurastClientLike {
  start(keyPair: AcurastKeyPair): Promise<void>;
  close(): void;
  send(recipient: string | Uint8Array, payload: string | Uint8Array): Promise<void>;
  onMessage: ((sender: string, payload: string) => void) | null;
}
 ...
 */
function buildPkcs8Der(rawKey: Uint8Array): Buffer {
  const header = Buffer.from(
    '304e020100301306072a8648ce3d020106082a8648ce3d030107043430320201010420',
    'hex'
  );
 ...
 */
function derivePublicKey(secretKeyBytes: Uint8Array): Uint8Array {
 ...
 */
export function generateP256KeyPair(): { secretKeyHex: string; publicKeyHex: string } {
...
export class AcurastMessagingClient {
  private acuClient: AcurastClientLike | null = null;
  private messageHandlers: Set<(msg: Message) => void> = new Set();
  private publicKeyHex: string = '';
  private wsUrl: string;

  constructor(wsUrl: string = ACURAST_WS_URL) {
   ...
   */
  async connect(secretKeyHex: string): Promise<void> {
    // Dynamic import — avoids breaking if @acurast/dapp is not installed at dev time
    let AcurastClient: new (url: string) => AcurastClientLike;
    try {
      ...
      throw new Error(
        `Failed to import @acurast/dapp — make sure it is installed:\n  npm install @acurast/dapp\n\nOriginal error: ${(err as Error).message}`
      );
    }
    ...
    this.acuClient.onMessage = (sender: string, payload: string) => {
      const msg: Message = {
        from: sender,
        payload: (() => {
          try {
