 ...
 */
export async function bundleEntryFile(entryPath, environment = {}) {
    ...
    try {
        ...
        throw new Error('esbuild is required to bundle deployment scripts. Install it with: npm install esbuild');
    }
    ...
    for (const [key, value] of Object.entries(environment)) {
        if (!SAFE_ENV_KEY_RE.test(key)) {
            throw new Error(`Invalid environment variable name in phonix.json: "${key}".\n` +
                'Keys must be SCREAMING_SNAKE_CASE (e.g. INFERENCE_API_URL).');
        }
        if (SECRET_KEY_PATTERNS.some((re) => re.test(key))) {
            throw new Error(`Environment variable "${key}" looks like a secret credential.\n` +
...
async function resolveAcurastCli() {
...
async function runAcurastCli(args, env) {
 ...
 */
function parseProcessorIds(output) {
    const ids = [];
 ...
 */
export async function acurastDeploy(options) {
    ...
    if (!mnemonic) {
        throw new Error('ACURAST_MNEMONIC is not set. Add it to your .env file.\n' +
    ...
    try {
        ...
        throw new Error(`Failed to bundle entry file '${config.code}':\n${err.message}`);
    }
    ...
    try {
        ...
        if (ipfsUrl)
            env['ACURAST_IPFS_URL'] = ipfsUrl;
        if (ipfsApiKey)
            env['ACURAST_IPFS_API_KEY'] = ipfsApiKey;
        const args = [
            'deploy',
            bundlePath,
        ...
        for (const dest of config.destinations ?? []) {
            if (!DEST_RE.test(dest)) {
                throw new Error(`Invalid destination value "${dest}". ` +
 ...
 */
export async function acurastEstimate(config) {
    try {
        const args = [
            'estimate-fee',
            '--replicas', String(config.replicas ?? 1),
 ...
 */
export async function acurastListDeployments(mnemonic) {
    const env = {};
