...
function assertSafeKoiiEndpoint(endpoint) {
    let parsed;
    try {
    ...
    if (parsed.protocol !== 'https:') {
        throw new PhonixError('koii', `Node endpoint must use https:// (got "${parsed.protocol}"). ` +
            'Plain HTTP would transmit payloads in cleartext.');
    }
    if (PRIVATE_HOST_RE.test(parsed.hostname)) {
        throw new PhonixError('koii', `Node endpoint hostname "${parsed.hostname}" resolves to a private/internal address. ` +
...
export class KoiiMessagingClient {
    ...
    messageHandlers = [];
    connected = false;
    constructor(rpcUrl = DEFAULT_KOII_RPC) {
        this.rpcUrl = rpcUrl;
    }
    async connect(secretKey, taskId) {
        ...
        try {
            const { Keypair } = (await import('@_koii/web3.js'));
            ...
            if (err.code === 'ERR_MODULE_NOT_FOUND') {
                ...
                console.warn('[phonix:koii] @_koii/web3.js not installed. ' +
    ...
    async send(nodeEndpoint, payload) {
        ...
        try {
            ...
            throw new PhonixError('koii', `Failed to reach Koii task node at ${nodeEndpoint}: ${err.message}`);
        }
        if (!submitResponse.ok) {
            throw new PhonixError('koii', `Task node returned ${submitResponse.status}: ${await submitResponse.text()}`);
        }
        ...
        if (result !== null) {
            ...
            if (result.length > MAX_RESULT_BYTES) {
                throw new PhonixError('koii', `Task node response exceeded ${MAX_RESULT_BYTES} bytes (got ${result.length} bytes).`);
            }
            const msg = {
                from: nodeEndpoint,
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function buildTaskUrl(nodeEndpoint, taskId, path) {
    const base = nodeEndpoint.replace(/\/$/, '');
    return taskId
        ? `${base}/task/${taskId}/${path}`
        : `${base}/${path}`;
}
async function pollForResult(url, timeoutMs, intervalMs) {
...
function base58OrHexToUint8Array(input) {
    ...
    if (/^[0-9a-fA-F]+$/.test(hex) && hex.length % 2 === 0) {
        ...
        if (!VALID_KEY_LENGTHS.has(byteLen)) {
            throw new PhonixError('koii', `Hex key must decode to exactly 32 or 64 bytes. Got ${byteLen} bytes.\n` +
...
function base58Decode(input) {
    ...
    for (const char of input) {
        ...
        if (idx < 0)
            throw new PhonixError('koii', `Invalid base58 character in key: '${char}'`);
    ...
    if (hex.length < 64) {
        throw new PhonixError('koii', `Base58 key decodes to only ${Math.ceil(hex.length / 2)} bytes — expected 32 or 64.\n` +
            'The key may be truncated or invalid. Run: phonix auth koii  to generate a new key.');
    }
    const byteLen = Math.ceil(hex.length / 2);
    if (!VALID_KEY_LENGTHS.has(byteLen)) {
        throw new PhonixError('koii', `Decoded key is ${byteLen} bytes — expected exactly 32 or 64 bytes.`);
    }
    ...
    if (parsed !== null && typeof parsed === 'object') {
        for (const key of Object.keys(parsed)) {
            if (key === '__proto__' || key === 'constructor' || key === 'prototype') {
                throw new PhonixError('koii', `Rejected remote payload: contains prototype-polluting key "${key}".`);
            }
        }
    }
    return parsed;
