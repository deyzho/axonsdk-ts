...
// ─── esbuild bundling ─────────────────────────────────────────────────────────
async function bundleForFluence(entryPath, environment = {}) {
    let esbuild;
    try {
...
// ─── CLI helpers ──────────────────────────────────────────────────────────────
async function resolveFluenceCli() {
    try {
...
async function runFluenceCli(args, env) {
...
export async function fluenceDeploy(options) {
    ...
    const privateKey = options.secretKey ??
        ...
        '';
    if (!privateKey) {
        throw new Error('FLUENCE_PRIVATE_KEY is not set. Add it to your .env file.\n' +
    ...
    try {
        ...
        throw new Error(`Failed to bundle for Fluence: ${err.message}`);
    }
    ...
    try {
        ...
        if (process.env['FLUENCE_NETWORK'])
            env['FLUENCE_ENV'] = process.env['FLUENCE_NETWORK'];
        const args = ['deploy', '--spell', spellPath, '--no-input'];
        if (config.replicas)
            args.push('--workers', String(config.replicas));
 ...
 */
export async function fluenceEstimate(config) {
    try {
 ...
 */
export async function fluenceListDeployments(secretKey) {
    const env = {};
    const key = secretKey ?? process.env['FLUENCE_PRIVATE_KEY'] ?? '';
    if (key)
        env['FLUENCE_PRIVATE_KEY'] = key;
    try {
