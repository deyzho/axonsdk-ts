...
- ✅ `@phonixsdk/inference` — OpenAI-compatible inference handler with latency-based routing
...
- ✅ `@phonixsdk/mobile` — React Native / Expo SDK with `useAxon`, `useMessages`, `useSend` hooks; `AxonProvider` context; iOS Keychain / Android Keystore; AppState lifecycle management
...
- [ ] **`@phonixsdk/llm`** — unified LLM client routing across Claude, Gemini, GPT-4, and self-hosted models
...
Axon follows [Semantic Versioning](https://semver.org). The public API is the `AxonClient` interface in `@phonixsdk/sdk` and the `axon` CLI commands. Breaking changes to either will increment the major version.
